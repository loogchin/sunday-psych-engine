...
{
	public static var noteoffset:Bool = false;

	var daSection:Int = 1;

	var susLength:Float;
	var floorSus:Int;
	var daStrumTime:Float;
	var daNoteData:Int;
	...
	override public function create()
	{
		...
		if (SONG.arrowSkin != null)
			guitarModNOW = SONG.arrowSkin.startsWith('GH_NOTES');
			...
			case 'garage':
				{
					...
					//dad = new Character(dad.x, dad.y, 'sunday-guitar-note');
				}
		}
	...
	public function startVideo2(name:String,?dialogueBox:DialogueBox):Void {
		#if VIDEOS_ALLOWED
	...
	public function startCountdown():Void
	{
		...
		if(ret != FunkinLua.Function_Stop) {
			switch(Paths.formatToSongPath(SONG.song)){
				default:
					generateStaticArrows(0,'normal');
					generateStaticArrows(1,'normal');
			}
	...
	private function generateStaticArrows(player:Int, style:String,tweenShit:Bool = true):Void
	{
		for (i in 0...4)
		{
			...
			babyArrow = new StrumNote(ClientPrefs.middleScroll ? STRUM_X_MIDDLESCROLL : STRUM_X, strumLine.y, i, player, style);
					...
					if(daNote.noteType == 'Hey!' && dad.animOffsets.exists('hey')) {
						...
						if (SONG.notes[Math.floor(curStep / 16)] != null)
						{
							if (SONG.notes[Math.floor(curStep / 16)].altAnim || daNote.noteType == 'Alt Animation') {
								altAnim = '-alt';
								if (SONG.song.toLowerCase() == 'marx' && !guitarModNOW)
									changeToGuitar();
									//changeCharacter('1', 'sunday-guitar-note');
							}else{
								if (SONG.song.toLowerCase() == 'marx' && guitarModNOW)
									altAnim = '';
									changeToSing();
									//changeCharacter('1', 'sunday-guitar');
							}
						}
	...
	override function beatHit()
	{
		...
		if (SONG.song.toLowerCase() == 'marx'){
			...
			if (curBeat == 224){
				...
				//dad.playAnim("end");
					dad.alpha = 0;
					var bruh:FlxSprite = new FlxSprite(dad.x,dad.y);
					bruh.frames = Paths.getSparrowAtlas("sunday/sunday_guitar_assets");
					bruh.animation.addByPrefix("bruh", "sunday guitar end", 24, false);
					bruh.animation.play("bruh");
					bruh.x -= 100;
					add(bruh);
				}else{
					endSong();
	...
	function changeToSing(){
		...
		//changeArrow('NOTE_assets');
		SONG.arrowSkin = 'NOTE_assets';
	
	}
	function changeCharacter(char:String, id:String)
		{				
			var charType:Int = Std.parseInt(char);
				if(Math.isNaN(charType)) charType = 0;

				switch(charType) {
					case 0:
						if(boyfriend.curCharacter != id) {
							if(!boyfriendMap.exists(id)) {
								addCharacterToList(id, charType);
							}

							boyfriend.visible = false;
							boyfriend = boyfriendMap.get(id);
							if(!boyfriend.alreadyLoaded) {
								boyfriend.alpha = 1;
								boyfriend.alreadyLoaded = true;
							}
