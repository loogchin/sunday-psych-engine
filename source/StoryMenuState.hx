...
class StoryMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		bgYellow = new FlxSprite(0, 56).makeGraphic(FlxG.width, 386, 0xFFFCFCFC);
		...
		for (i in 0...CoolUtil.difficultyStuff.length) {
			sprDifficulty = new FlxSprite(leftArrow.x + 60, leftArrow.y).loadGraphic(Paths.image('menudifficulties/' + CoolUtil.difficultyStuff[i][0].toLowerCase()));
	...
	override function update(elapsed:Float)
	{
		...
		{
			FlxTween.tween(tracksSprite, {x:-1300,alpha:0}, 0.1,{ease:FlxEase.quadIn});
			FlxTween.tween(txtTracklist, {y:730,alpha:0}, 0.1,{ease:FlxEase.quadIn});
			FlxTween.tween(scoreText, {x:1300}, 0.1,{ease:FlxEase.quadIn});
			FlxTween.tween(bgYellow, {x:1300}, 0.1,{ease:FlxEase.quadIn});
